...
def test_get_premium_quotes_filters(mock_get_db):
    ...
    result = get_premium_quotes(age=25, company_code="aia", plan_code="basic", max_premium=5000000, limit=10)
...
def test_search_glossary_terms(mock_get_db):
    ...
    mock_cursor.fetchall.return_value = [("Bệnh có sẵn", "Tình trạng sức khỏe có trước ngày hiệu lực.")]

    result = search_glossary_terms("bệnh có sẵn", company_code="aia", limit=5)

    assert result == [{"term": "Bệnh có sẵn", "definition": "Tình trạng sức khỏe có trước ngày hiệu lực."}]
    query, params = mock_conn.execute.call_args.args
...
def test_compare_benefits_filters(mock_get_db):
    ...
    mock_cursor.fetchall.return_value = [("aia", "100.000.000"), ("liberty", "120.000.000")]

    result = compare_benefits("ung thư", ["aia", "liberty"], plan_codes=["basic"], limit=20)
    ...
    assert params == ["%ung thư%", "%ung thư%", "%ung thư%", "aia", "liberty", "basic", 20]
...
def test_search_exclusions_param_order(mock_get_db):
    ...
    assert params == [
        "%loại trừ%", "%loại trừ%", "%loại trừ%",
        "%không chi trả%", "%không chi trả%", "%không chi trả%",
        "%không được bảo hiểm%", "%không được bảo hiểm%", "%không được bảo hiểm%",
        "%exclusion%", "%exclusion%", "%exclusion%",
        "%implant%", "%implant%", "%implant%",
        "aia",
        "%loại trừ%", "%loại trừ%",
        "%không chi trả%", "%không chi trả%",
        "%không được bảo hiểm%", "%không được bảo hiểm%",
        "%exclusion%", "%exclusion%",
        "%implant%", "%implant%",
        "aia",
        7,
