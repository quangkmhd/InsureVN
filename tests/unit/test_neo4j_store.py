...
def test_neo4j_store_creates_uniqueness_constraints() -> None:
    ...
    assert any("Company_id_unique" in query for query in queries)
    assert any("Document_id_unique" in query for query in queries)
    assert any("Plan_id_unique" in query for query in queries)
